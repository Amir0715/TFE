...
class Question(BaseModel):
    ...
    variant_answers = models.ManyToManyField(VariantOfAnswer, through='QuestionVariantOfAnswer')
...
class QuestionVariantOfAnswer(BaseModel):
    ...
    question = models.ForeignKey('Question', on_delete=models.RESTRICT, verbose_name='Вопрос', related_name='')
    variant_of_answer = models.ForeignKey('VariantOfAnswer', on_delete=models.RESTRICT, verbose_name='Вариант ответа')
...
class AnswersFromHuman(BaseModel):
    ...
    question = models.ForeignKey(
        'Question', on_delete=models.RESTRICT, verbose_name='Вопрос на который отвечают')
    # answers_from_user Many to Many - так как может быть много ответов на один вопрос.
    answers_from_user = models.ManyToManyField(VariantOfAnswer, verbose_name='Вариант ответа пользователя')
    complited_test = models.ForeignKey('ComlitedUserTest', on_delete=models.RESTRICT, verbose_name='Принадлежит тесту', related_name='answers')
    
class ComplitedUserTest(BaseModel):
    test = models.ForeignKey('Test', on_delete=models.RESTRICT, verbose_name='Пройденный тест', related_name='complited_tests')
    ...
    score_from_test = models.PositiveSmallIntegerField(verbose_name='Количество баллов за тест')
...
class Test(BaseModel):
    ...
    question = models.ManyToManyField('Question', verbose_name='Вопрос', related_name='tests')
